                ...
                <div class="rounded rounded-lg p-4 bg-light">
                    <form method="POST" action="{{ route($routeIndex . '.update', $detail->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="card mb-3 border p-2" style="max-width: 640px;">
                            <div class="row g-0">
                                <div class="col-md-4">
                                    <img id="imageThumbnail" src="{{old('image', $detail->image) ?? asset('images/no-image.jpeg')}}" class="img-thumbnail rounded" alt="Photo" width="200px" height="200px">
                                </div>
                                <div class="col-md-8">
                                    <div class="card-body py-2">
                                        ...
                                        <input type="hidden" name="image" id="imageValue" value="{{old('image', $detail->image)}}">
                                        <button style="{{old('image', $detail->image) ? 'display: none;' : ''}}" class="btn btn-outline-brand mt-5" id="btnUpload" type="button" onclick="document.getElementById('inputImage').click()">
                                            Unggah Gambar
                                        </button>
                                        <div style="{{old('image', $detail->image) ? '' : 'display: none;'}}" class="label-group">
                                            <p id="imageLabel">&nbsp;</p>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('name') {{'is-invalid'}} @enderror" id="inputName" aria-describedby="name" name="name" value="{{old('name', $detail->name)}}" placeholder="Nama Produk">
                            @error('name')
                                <div class="form-text text-danger">*{{$message}}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select id="selectCategory" class="form-select @error('category_id') {{'is-invalid'}} @enderror" name="category_id" data-placeholder="Pilih Kategori">
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{$category->id}}" {{ old('category_id', $detail->category_id) == $category->id ? 'selected' : '' }}>{{$category->name}}</option>
                                @endforeach
                              </select>
                            @error('category_id')
                                <div class="form-text text-danger">*{{$message}}</div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="inputDescription" class="form-control @error('description') {{'is-invalid'}} @enderror" name="description" placeholder="Deskripsi Produk" rows="15">{{old('description', $detail->description)}}</textarea>
                            @error('description')
                                <div class="form-text text-danger">*{{$message}}</div>
                            @enderror
                        </div>
